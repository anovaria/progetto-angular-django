...
Write-Host "       DEPLOY COMPLETO - AMBIENTE TEST          " -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
# ============================================
Write-Host "[4/6] Build Angular (configurazione test)..." -ForegroundColor Yellow

Push-Location $FrontendSource

$ErrorActionPreference = "Continue"
ng build --configuration=test 2>&1 | Out-Null
    ...
    <rewrite>
      <rules>
        ...
        <rule name="Admin Proxy" stopProcessing="true">
          <match url="^admin/(.*)" />
          <action type="Rewrite" url="http://localhost:8001/admin/{R:1}" />
        </rule>
        <rule name="Dashboard Proxy" stopProcessing="true">
          <match url="^dashboard/(.*)" />
          <action type="Rewrite" url="http://localhost:8001/dashboard/{R:1}" />
        </rule>
        <rule name="Dashboard Root" stopProcessing="true">
          <match url="^dashboard$" />
          <action type="Rewrite" url="http://localhost:8001/dashboard/" />
        </rule>
        <rule name="Rossetto Proxy" stopProcessing="true">
          <match url="^rossetto/(.*)" />
          <action type="Rewrite" url="http://localhost:8001/rossetto/{R:1}" />
        </rule>
        <rule name="Rossetto Root" stopProcessing="true">
          <match url="^rossetto$" />
          <action type="Rewrite" url="http://localhost:8001/rossetto/" />
        </rule>
