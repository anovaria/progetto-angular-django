...
def _load_ean_set() -> set[str]:
    """Load all EAN values from the compact GoldTableSnapshot for dbo.t_t_Ean."""
    snap = GoldTableSnapshot.objects.filter(source_table=T_EAN).only('payload').first()
    if not snap or not snap.payload:
        return set()
    eans = snap.payload.get('__ean_list__')
    if eans is not None:
        return {str(e).strip() for e in eans if e}
    # fallback: vecchio formato riga-per-riga
    s: set[str] = set()
    for snap in GoldTableSnapshot.objects.filter(source_table=T_EAN).only('payload').iterator(chunk_size=5000):
        p = snap.payload or {}
