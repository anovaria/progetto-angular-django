...
def dashboard_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(f"{url}?batch={batch.id}&skip_sync=1")
    else:
        form = ElabUploadForm()

    # In questa versione l'anteprima usa uno scroll verticale (stile versione originale)
    # quindi non mostriamo più il dettaglio completo in fondo pagina.

    gold_preview = None
    gold_sync_info = request.session.get('gold_sync_last')
    # Auto-sync all'apertura della dashboard (comportamento A),
    # ma evitiamo di rilanciarla dopo azioni che fanno redirect qui (upload/cancella/etc.).
    needs_gold_sync = request.GET.get('skip_sync') != '1'
    if request.GET.get('gold') == '1':
        gold_preview = get_preview_from_local(limit=5)

    # Anteprima tabellare (finestra alta ~10 righe) con scroll verticale,
    # in modo da poter scorrere tutte le righe come nella versione originale.
    preview_columns = [
        'line_number', 'cod_art_fo', 'descrizione_articolo', 'iva', 'prz_acq',
        'pz_x_crt', 'crt_x_str', 'str_x_plt', 'tot_colli', 'ean'
    ]
    preview_headers = [
        'Riga', 'CodArtFo', 'Descrizione', 'IVA', 'PrzAcq',
        'PzXCrt', 'CrtXStr', 'StrXPlt', 'TotColli', 'EAN'
    ]
    preview_rows = []
    preview_total = 0
    ...
    if selected_batch:
        try:
            qs_prev = selected_batch.rows.all().order_by('line_number')
            preview_total = qs_prev.count()
            # Salvaguardia: se un file ha decine di migliaia di righe, evitare pagine troppo pesanti.
            max_preview = 10000
            if preview_total > max_preview:
                qs_prev = qs_prev[:max_preview]
                preview_truncated = True

            for r in qs_prev.iterator(chunk_size=2000):
                preview_rows.append({
                    'line_number': str(r.line_number).zfill(4),
                    'cod_art_fo': r.cod_art_fo,
                    'descrizione_articolo': r.descrizione_articolo,
                    'iva': r.iva if r.iva is not None else '',
                    'prz_acq': r.prz_acq if r.prz_acq is not None else '',
                    'pz_x_crt': r.pz_x_crt if r.pz_x_crt is not None else '',
                    'crt_x_str': r.crt_x_str if r.crt_x_str is not None else '',
                    'str_x_plt': r.str_x_plt if r.str_x_plt is not None else '',
                    'tot_colli': r.tot_colli if r.tot_colli is not None else '',
                    'ean': r.ean,
                })
        except Exception:
            preview_rows = []
    ...
    context = {
        ...
        'needs_gold_sync': needs_gold_sync,
...
def delete_batch_view(request, pk: int):
    ...
    if request.method == "POST":
        batch.delete()
        return redirect(f"{reverse('importelab:dashboard')}?skip_sync=1")
...
def sync_gold_view(request):
    ...
    # Supporto AJAX: usato dalla dashboard per avviare la sync automaticamente
    wants_json = (
    ...
    if wants_json:
        preview = get_preview_from_local(limit=5)
        return JsonResponse({
            'ok': True,
            'synced_at': request.session['gold_sync_last']['synced_at'],
            'summary': request.session['gold_sync_last']['summary'],
            'preview': preview,
        })

    url = reverse('importelab:dashboard')
    return redirect(f"{url}?gold=1&skip_sync=1")
...
def regen_intermediate_view(request):
    ...
    if batch_id:
        ...
        if not batch:
            url = reverse('importelab:dashboard')
            return redirect(f"{url}?skip_sync=1")
    ...
    url = reverse('importelab:dashboard')
    return redirect(f"{url}?batch={batch.id}&inter=1&skip_sync=1")
...
def ordine_email_view(request):
    ...
    # NOTA UX: non generiamo automaticamente su GET.
    # L'utente decide in POST se riusare un ordine esistente o rigenerarlo.
    attachment_exists = os.path.exists(attachment_path)

    if request.method == "POST":
        # Default: rigenera (è il caso più frequente). Se deselezionato, riusa l'ordine presente.
        force_regen = request.POST.get("force_regen") == "1"

        # Se richiesto, cancella il file precedente (se presente) e rigenera.
        # Se il file non esiste, rigenera comunque.
        if force_regen or not os.path.exists(attachment_path):
            try:
                if force_regen and os.path.exists(attachment_path):
                    try:
                        os.remove(attachment_path)
                    except Exception:
                        # Se non riesco a cancellare, provo comunque a rigenerare
                        pass
                attachment_path = generate_ordine_ext_file(source_dir)
                attachment_exists = True
                message = "Ordine rigenerato da GoldReport."
            except Exception as e:
