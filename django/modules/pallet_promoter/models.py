...
class Fornitore(models.Model):
    """
    Fornitori - LETTI DA GOLDREPORT (ERP Gold, read-only).
    I dati sono sempre aggiornati automaticamente dall'ERP.
    """
    codice = models.IntegerField(primary_key=True, db_column='FCODFO')
    nome = models.CharField(max_length=50, db_column='FNOMFO')
    buyer = models.CharField(max_length=50, blank=True, null=True, db_column='BUYER')
    reparto = models.CharField(max_length=13, blank=True, null=True, db_column='FREPAR')
    codice_commerciale = models.CharField(max_length=8, blank=True, null=True, db_column='CCOM')
...
class AssegnazionePallet(models.Model):
    ...
    @property
    def fornitore(self):
        """Recupera oggetto Fornitore da goldreport."""
...
# ========================================
# MODELLI ATTIVITÀ NON PREVISTE
# ========================================

class SlotPolmone(models.Model):
    """
    Anagrafica nominativi per Attività non previste.
    Persone che entrano per rilevamenti prezzi, ecc.
    """
    id = models.IntegerField(primary_key=True)
    nominativo = models.CharField(max_length=100)
    note = models.TextField(blank=True, null=True)
    da_eliminare = models.BooleanField(default=False)

    class Meta:
        managed = False
        db_table = 'shared.slot_polmone'
        verbose_name = "Nominativo Attività"
        verbose_name_plural = "Nominativi Attività"
        ordering = ['nominativo']
...
class IngressoUscitaPolmone(models.Model):
    """
    Registrazione orari giornalieri per Attività non previste.
    """
    id = models.AutoField(primary_key=True)
    slot_polmone = models.ForeignKey(
        SlotPolmone, 
        on_delete=models.CASCADE,
        db_column='slot_polmone_id',
    ...
    ingresso_extra_1 = models.TimeField(blank=True, null=True)
    uscita_extra_1 = models.TimeField(blank=True, null=True)
    ingresso_extra_2 = models.TimeField(blank=True, null=True)
    uscita_extra_2 = models.TimeField(blank=True, null=True)
    forzato = models.BooleanField(default=False)

    class Meta:
        managed = False
        db_table = 'shared.ingressi_uscite_polmone'
        verbose_name = "Orario Attività"
        verbose_name_plural = "Orari Attività"
        ordering = ['-data']
        unique_together = ['slot_polmone', 'data']

    def __str__(self):
        return f"{self.slot_polmone.nominativo} - {self.data}"
    ...
    def ore_totali(self):
        ...
        if self.ingresso_1 and self.uscita_1:
            delta = timezone.datetime.combine(timezone.now().date(), self.uscita_1) - \
                    timezone.datetime.combine(timezone.now().date(), self.ingresso_1)
            ore += delta.total_seconds() / 3600
        
        if self.ingresso_2 and self.uscita_2:
            delta = timezone.datetime.combine(timezone.now().date(), self.uscita_2) - \
                    timezone.datetime.combine(timezone.now().date(), self.ingresso_2)
            ore += delta.total_seconds() / 3600
        
        if self.ingresso_extra_1 and self.uscita_extra_1:
            delta = timezone.datetime.combine(timezone.now().date(), self.uscita_extra_1) - \
                    timezone.datetime.combine(timezone.now().date(), self.ingresso_extra_1)
            ore += delta.total_seconds() / 3600
        
        if self.ingresso_extra_2 and self.uscita_extra_2:
            delta = timezone.datetime.combine(timezone.now().date(), self.uscita_extra_2) - \
                    timezone.datetime.combine(timezone.now().date(), self.ingresso_extra_2)
            ore += delta.total_seconds() / 3600
